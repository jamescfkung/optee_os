...
STABLE_OPTEE_OS_COMMIT=origin/mt8173_port

SRC_OPTEE_CLIENT=https://github.com/jamescfkung/optee_client.git
DST_OPTEE_CLIENT=$DEV_PATH/optee_client
STABLE_OPTEE_CLIENT_COMMIT=1499fae3d58d489dce12ee82331a96ed1271df8b
...
STABLE_OPTEE_TEST_COMMIT=678f375af17881a93e7061f3435d0f7f55688573
...
################################################################################
cd $DST_KERNEL
for p in pinctrl ccf pmic mmc i2c cpufreq cpuidle usb scp; do git am $DST_KERNEL_PATCHES/$p/*.patch; done
...
if [ ! -f ".config" ]; then
	sed -i '/config ARM$/a  select DMA_SHARED_BUFFER' arch/arm/Kconfig
...
(cd mtk_tools &&
./build_trustzone.sh $DST_OPTEE_OS/out/arm32-plat-mediatek/core/tee-pager.bin &&
